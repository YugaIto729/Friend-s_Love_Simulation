...
public class ConditionParser : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        string puttern = "0.1+0.8";
        var s = new Parser();
        s.Start_Value(puttern);
...
public sealed class Parser
{
    ...
    public int Eval_Value(float[] values)
    {
        ...
        {
            return (int)Analysis_formale(new Node(side_formale[0]), values);
        }
    ...
    public bool Eval_old(float[] values)
    {
        float left = Analysis_formale(new Node(side_formale[0]), values);
        float right = Analysis_formale(new Node(side_formale[1]), values);
    ...
    public bool Eval_Multi(float[] values)
    {
        return Analysis_formale_Multi(root, values);
    }

    private bool Analysis_formale_Multi(Node baseNode, float[] values)
    {
        bool left;
        bool right;
        //int index_child = 0;
    ...
    private float Analysis_formale(Node node, float[] values)
    {
        float left = 0;
        float right = 0;
        int index_child = 0;
        ...
        {
            outValue = float.Parse(node.numbers[0]);
        }
        //Debug.Log("答え: " + outValue);
        return outValue;
    }

    private bool Side_Nomber_Multi(Node node, int index, float[] values)
    {
        bool outBool = false;
        ...
        if (index < node.parent.numbers.Count)
        {
            ...
            else
            {
                ...
                */
                ...
                float left = Analysis_formale(new Node(node.children[0]), values);
                float right = Analysis_formale(new Node(node.children[1]), values);
    ...
    private float Side_Nomber(Node node, int index, ref int index_child, float[] values)
    {
        float outNumber = 0;
        ...
        if (index < node.numbers.Count)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        if (float.Parse(node.numbers[index]) < values.Length)
                        {
                    ...
                    {
                        outNumber = float.Parse(node.numbers[index]);
                    }
                }
                catch
                {
    ...
    public void Start_Value(string formale)
    {
        ...
        string fixed_formale = ReplaceReciprocal(EraseSpace(formale));
        Debug.Log("式: " + formale);
    ...
    public void Start_Multi(string condition)
    {
        ...
        Node node = new Node();
        root = node;
        string formale = ReplaceReciprocal(EraseSpace(condition));
        //Debug.Log("式: " + formale);

        if (formale == "") formale = "0==0";

        Node_Register_Multi(formale, node);
    ...
    private string ReplaceReciprocal(string text)
    {
        //-2*-2
        //-1-2
        //1-1
        return Regex.Replace(text, @"\\rv", "(1-2)");
    }
